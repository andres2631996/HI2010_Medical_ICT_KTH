...
%%%%%%%%%%% set testmode 2 to show graphics  
testmode = 2;

%%%%%%%%%%% make threshold 0.2 -> this was 0.15 on MIT data
thresh = 0.2;

%%%%%%%%%%% make sample frequency  256 Hz
samp_freq = 256;
...
%%%% Q-wave detection: Assume is the minimum value between the rise of the
%%%% QRS and the R peak
for(i=1:length(left))
  [Qval(i), Qloc(i)] = min( bpf((left(i)-13):maxloc(i)) );
  Qloc(i) = Qloc(i)-14+left(i); % add offset of present location
end
...
if (testmode~=0)
    ...
    plot(R_t,R_amp,'+k');
    plot(S_t,S_amp,'+g');    
    title('ECG with Q peaks (blue), R-peaks (black) and S-points (green) over ECG')
    ylabel('ECG+Q+R+S')
    hold off;
    subplot(4,1,4)
    hold off
